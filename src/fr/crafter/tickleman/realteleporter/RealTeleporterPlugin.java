			...
			if (command.equals("rtel") || command.equals("rt")) {
				...
				if (hasPermission(player, "realteleporter." + param1)) {
					switch (param1) {
						...
						case "link": {
							...
							if (teleporter1 != null && teleporter2 != null) {
								...
								if (teleporter1 == null)
									player.sendMessage(tr("Teleporter +1 does not exist").replace("+1", param2));
								if (teleporter2 == null)
									player.sendMessage(tr("Teleporter +1 does not exist").replace("+2", param3));
							}
							return true;
						}
						case "loop": {
							...
							if (teleporter1 != null && teleporter2 != null) {
								...
								if (teleporter1 == null)
									player.sendMessage(tr("Teleporter +1 does not exist").replace("+1", param2));
								if (teleporter2 == null)
									player.sendMessage(tr("Teleporter +1 does not exist").replace("+1", param3));
							}
							return true;
						}
						case "unlink": {
						...
						case "list": {
							// list teleporters
							StringBuilder values = new StringBuilder(tr("Gates list")).append(" :");
							for (RealTeleporter teleporter : teleporters.byName.values()) {
								values.append(" ").append(teleporter.name);
							}
						...
						case "orphan": {
							...
							StringBuilder values = new StringBuilder(tr("Orphan gates")).append(" :");
							for (RealTeleporter teleporter1 : teleporters.byName.values()) {
								if (teleporter1.target == null) {
									...
									if (!found) {
										values.append(" ").append(teleporter1.name);
									}
								}
							}
							player.sendMessage(values.toString());
						...
						case "withouttarget": {
							...
							StringBuilder values = new StringBuilder(tr("Gates without target")).append(" :");
							for (RealTeleporter teleporter : teleporters.byName.values()) {
								if (teleporter.target == null) {
									values.append(" ").append(teleporter.name);
								}
						...
						case "withoutsource": {
							...
							StringBuilder values = new StringBuilder(tr("Gates without source")).append(" :");
							for (RealTeleporter teleporter1 : teleporters.byName.values()) {
								boolean found = false;
								for (RealTeleporter teleporter2 : teleporters.byName.values()) {
									if (teleporter2.targetName.equals(teleporter1.name)) {
